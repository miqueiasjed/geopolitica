...
3. Leia `.claude/rules.md` e o `CLAUDE.md` para relembrar as convenções.
4. Se o plano referencia um fragmento do PRD (ex: `.claude/prd/agendamentos.md`), leia-o.
...
A IA deve:
1. Ler `.claude/plans/5.md`.
2. Decompor em tasks granulares.
3. Criar os arquivos em `.claude/tasks/5/`.
...
1. Leia `.claude/plans/INDEX.md`.
2. Para cada plano com status ⏳ que ainda não tem pasta em `.claude/tasks/`:
   - Execute esta skill.
