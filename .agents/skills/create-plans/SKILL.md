---
name: create-plans
description: Lê um PRD/SRS e gera planos numerados em .Codex/plans/, atualizando o INDEX.md automaticamente.
---
...
1. Crie a pasta `.Codex/prd/` (se não existir).
...
1. Leia o PRD completo (ou os fragmentos em `.Codex/prd/`).
2. Leia `.Codex/plans/INDEX.md` para saber quais planos já existem e qual é o próximo número disponível.
3. Leia `.Codex/rules.md` (se existir) para respeitar restrições do projeto.
...
Para cada módulo identificado, crie um arquivo `.Codex/plans/[N].md` com esta estrutura:
...
[Se o PRD foi fragmentado: "Detalhes completos em: `.Codex/prd/[modulo].md`"]
...
Após gerar todos os planos, atualize (ou crie) `.Codex/plans/INDEX.md`:
...
3. Gerar os planos em `.Codex/plans/`.
